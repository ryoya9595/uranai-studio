...
2. **Add** で以下を登録：
   - `GEMINI_API_KEY` … 種別 **Secret（暗号化）** に、手順0でコピーしたキーを貼る（必須）
   - `ALLOWED_ORIGIN` … 種別 Variable（任意・推奨）。自分のサイトのオリジン（例 `https://<あなたのGitHubユーザー名>.github.io`）を入れると、そこ以外からの呼び出しをブロックする。未設定なら制限なし
